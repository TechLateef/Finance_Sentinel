const HIVE_API = 'http://127.0.0.1:8787';
...
async function init() {
  ...
  try {
    ...
    const discover = await fetch(`${HIVE_API}/api/sessions`).then(r => r.json());
    const sentinelSession = discover.sessions.find(s => s.worker_name === 'Finance Sentinel');

    if (sentinelSession) {
      currentSessionId = sentinelSession.session_id;
      log(`Reconnected to active session: ${currentSessionId}`);
    } else {
      log('No active session found. Initializing new Finance Sentinel context...');
...
async function startNewSession() {
  try {
    const resp = await fetch(`${HIVE_API}/api/sessions`, {
      ...
      body: JSON.stringify({
        agent_path: '/home/mk_lateef/Desktop/OSS/hive/core/framework/agents/finance_sentinel'
      })
    }).then(r => r.json());
...
async function startPolling() {
  ...
  setInterval(async () => {
    ...
    try {
      const msgs = await fetch(`${HIVE_API}/api/sessions/${currentSessionId}/queen-messages`).then(r => r.json());
  ...
  setInterval(async () => {
    if (!currentSessionId) return;
    try {
      const progress = await fetch(`${HIVE_API}/api/sessions/${currentSessionId}/goal-progress`).then(r => r.json());
      if (progress && progress.total_weight > 0) {
          const percent = Math.min(100, (progress.accomplished_weight / progress.total_weight) * 100);
...
function updateTerminal(messages) {
  ...
  messages.forEach(m => {
    if (m.seq > lastMessageSeq) {
      ...
      if (m.role === 'assistant') {
        const type = m.content.toLowerCase().includes('approved') ? 'success' : 'thought';
        log(m.content, type);
...
btnEvaluate.addEventListener('click', async () => {
  ...
  log(`Dispatching Loan Evaluation: ${amount} USD₮ to ${borrower}...`, 'action');

  try {
    const resp = await fetch(`${HIVE_API}/api/sessions/${currentSessionId}/trigger`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
