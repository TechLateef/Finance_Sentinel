                ...
                switch (name) {
                    case "wdk_get_address": {
                        if (!this.walletManager)
                            throw new Error("Wallet not initialized. Check WDK_SEED_PHRASE.");
                    ...
                    case "wdk_get_balance": {
                        if (!this.walletManager)
                            throw new Error("Wallet not initialized. Check WDK_RPC_URL.");
                        let targetAddress = args?.address;
                        if (!targetAddress) {
                            const account = await this.walletManager.getAccount(0);
                            targetAddress = await account.getAddress();
                        }
                        const tokenAddress = args?.token_address || this.DEFAULT_USDT_CONTRACT;
                        const account = await this.walletManager.getAccount(0);
